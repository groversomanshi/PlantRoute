...
/** Map Activity to engine ActivityInput. */
export function activityToEngineInput(a: Activity): ActivityInput {
  return {
    id: a.id,
    name: a.name,
    category: (a.category ?? "outdoor").toLowerCase(),
...
/** Simple interest match: 1 if category in interests (or no interests), else 0. */
function simpleInterestMatch(interests: string[], category: string): number {
  ...
  return normalized.includes(cat) ? 1 : 0;
}
