...
export function CitySearchBar({
  ...
  useEffect(() => {
    ...
    if (suggestions.length > 0) setHighlightedIndex(0);
  ...
  return (
    <div className="w-full max-w-2xl relative" ref={dropdownRef}>
      ...
      {dropdownOpen && (suggestions.length > 0 || suggestionsLoading) && (
        <ul
          id="city-suggestions"
          role="listbox"
        ...
        >
          {suggestionsLoading ? (
            <li className={`px-4 py-3 text-sm ${theme.dropdownItem} opacity-70`}>
              Finding places…
            </li>
          ) : (
