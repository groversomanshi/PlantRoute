    ...
    for _ in range(n):
        out.append(
            TravelPreferencesInput(
                ...
                eco_preference=float(np.clip(np.random.beta(2, 2), 0.05, 0.95)),
            )
        )
    return out
...
def heuristic_regret(
    ...
    emission_fit = feats.get("emission_fit", 0.5)
    ...
    regret = (1.0 - im) * 0.28 + (1.0 - emission_fit) * 0.22 + crowd * 0.12 + early * 0.12 + late * 0.08 + budget * 0.12 + pace * 0.06
...
def generate_dataset(n_samples: int = 10_000) -> pd.DataFrame:
    ...
    for i in range(n_samples):
        ...
        # 50/50 target regret; random (interest, category) pairs give continuous interest_match from similarity matrix
        target_regret = i % 2
        ...
        if target_regret == 1:
            ...
            cat = random.choice(interests) if interests else random.choice(ATTRACTION_TYPES)
            activity = sample_activity(category=cat)
        feats = build_features(travel, interests, activity)
