...
from ml.preference_engine.features import build_features
from ml.preference_engine.schemas import Context, ItineraryItem, UserPreferences
...
def test_higher_cost_increases_budget_overrun() -> None:
    prefs = UserPreferences(budget_comfort=0.3)
    item_lo = ItineraryItem(cost_level=0.2)
    item_hi = ItineraryItem(cost_level=0.9)
    f_lo = build_features(prefs, item_lo, None)
    f_hi = build_features(prefs, item_hi, None)
    assert f_hi["budget_overrun"] >= f_lo["budget_overrun"]


def test_outdoor_bad_weather_flag() -> None:
    prefs = UserPreferences(dislike_rain=True)
    item_out = ItineraryItem(outdoor_fraction=1.0, bad_weather_today=True)
    item_in = ItineraryItem(outdoor_fraction=0.0, bad_weather_today=True)
    f_out = build_features(prefs, item_out, None)
    f_in = build_features(prefs, item_in, None)
    assert f_out["outdoor_bad_weather"] > f_in["outdoor_bad_weather"]


def test_late_night_after_early_with_context() -> None:
    prefs = UserPreferences()
    item = ItineraryItem(start_hour=6.0, is_late_night=True)
    ctx = Context(previous_day_end_hour=23.0)
    feats = build_features(prefs, item, ctx)
    assert "late_night_after_early" in feats
