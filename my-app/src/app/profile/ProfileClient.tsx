...
export default function ProfileClient() {
  const { userId, profile, loading, load, savePreferences } = useProfile();
  const [preferences, setPreferences] = useState<UserPreferences | null>(null);
  const [pastTrips, setPastTrips] = useState<Array<{ id: string; city: string; date: string; kg: number }>>([]);
  ...
  useEffect(() => {
    ...
    setPastTrips(
      list.map((it: { id: string; city: string; start_date: string; total_emission_kg: number }) => ({
        id: it.id,
        city: it.city,
        date: it.start_date,
